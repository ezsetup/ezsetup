<template>
  <table class="table is-striped">
    ...
    <tbody>
      <tr v-for="report in reports" track-by="$index">
        ...
        <td><router-link v-if="detailsDisplay(report.id)" class="button is-small" :to="{ name: 'GradeDetails', params: { id: report.id } }">Grade Details</router-link></td>
      </tr>
    </tbody>
  </table>
</template>
    ...
    created: function () {
      LISTReports (json => {
        for (var i = 0; i < json.length; i++) {
          if (json[i].student === this.userInfo.fullname) {
            this.reports.push(json[i])
          }
        }
        LISTGrades (json=> {
          for (var i = 0; i < json.length; i++) {
            if (json[i].student === this.userInfo.fullname) {
              this.grades.push(json[i])
            }
          }
